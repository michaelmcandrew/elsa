...
op <- par(oma = c(0, 0, 0, 0), mfrow = c(3, 2), cex = 1, mar = c(2, 2, 2, 2), xpd = TRUE, cex = 0.7)
for (i in 1:length(measures)) {
	plot(wave0$survival[!is.na(wave0[,measures[i]])], resds[[i]][,1], main = measures[i], pch = ".")	
}
...
x.l <- split(tmpdat, f = tmpdat$cSES3)
